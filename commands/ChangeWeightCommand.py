...
class ChangeWeightCommand(commands.Cog):
    ...
    async def change_weight_command(self, ctx,
        ...
        async def update_embed():
            ...
            # Display relic_sets changes
            embed_value = ""
            relic_sets_data = await get_relic_sets()
            if "relic_sets" in base_weight_model and "relic_sets" in weight.model_dump():
                for relic_set in weight.relic_sets:
                    base_value = 0
                    base_num = 2  # Default num value
                    for base_relic_set in base_weight_model.get("relic_sets", []):
                        if base_relic_set["id"] == relic_set.id:
                            base_value = base_relic_set["weight"]
                            # Get num if it exists in base_relic_set
                            if "num" in base_relic_set:
                                base_num = base_relic_set["num"]
                            break
                    # Skip if both weight and num are unchanged
                    if base_value == relic_set.weight and base_num == relic_set.num:
                        continue
        ...
        class MyView(discord.ui.View):
            ...
            async def button_callback_send(self, button, interaction):
                ...
                # Handle relic_sets
                if "relic_sets" in base_weight_model:
                    weight_model["relic_sets"] = []
                    for relic_set in weight.relic_sets:
                        base_value = 0
                        base_num = 2  # Default num value
                        for base_relic_set in base_weight_model.get("relic_sets", []):
                            if base_relic_set["id"] == relic_set.id:
                                ...
                                if "num" in base_relic_set:
                                    base_num = base_relic_set["num"]
                                break
                        # Include the relic set if either weight or num has changed
                        if base_value != relic_set.weight or base_num != relic_set.num:
                            weight_model["relic_sets"].append({
                                "id": relic_set.id,
